...
def test_tensor_names_collected_correctly(simstate):
...
def test_exclude_mechanism(simstate):
...
def test_to_device_and_to_host_round_trip(simstate):
    handler = GPU_handler(simstate)
...
def test_non_array_fields_untouched(simstate):
    handler = GPU_handler(simstate)
